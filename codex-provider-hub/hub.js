...
function textFromResponsePayload(payload) {
  if (typeof payload?.output_text === "string" && payload.output_text.trim()) return payload.output_text.trim();
  if (Array.isArray(payload?.output)) {
    for (const item of payload.output) {
      if (Array.isArray(item?.content)) {
        const text = item.content.map((part) => part?.text || part?.content || "").filter(Boolean).join(" ").trim();
        if (text) return text;
      }
  ...
  if (Array.isArray(payload?.choices)) {
    const text = payload.choices.map((choice) => choice.message?.content || choice.text || "").filter(Boolean).join(" ").trim();
    if (text) return text;
  }
  return payload?.error?.message || payload?.raw || "";
}

function compactSample(text, limit = 120) {
...
async function handleApi(req, res) {
  ...
  if (req.method === "POST" && pathname === "/api/gateway-toggle") {
    ...
    sendJson(res, 200, { ok: true, enabled: true, message: "代理已启动：Codex 将通过本地代理使用当前厂商。若 Codex 已经在运行，重启一次后进入代理；之后切换厂商无需再退出 Codex。" });
    return;
  }
  ...
  if (req.method === "POST" && pathname === "/api/switch") {
    ...
    const wasGatewayEnabled = config.gatewayEnabled === true;
    try {
    ...
    sendJson(res, 200, {
      ok: true,
      gatewayEnabled: true,
      hotSwitch: wasGatewayEnabled,
      message: wasGatewayEnabled
        ? `已热切换到 ${provider.displayName || provider.id}：下一次 Codex 请求直接使用新厂商，无需退出 Codex。`
        : `已切换到 ${provider.displayName || provider.id} 并启动代理。Codex 若仍在官方模式，重启一次后即可；之后切换厂商无需再退出 Codex。`
    });
    return;
  }
  ...
  if (req.method === "POST" && pathname === "/api/test-active") {
    ...
    const sample = compactSample(textFromResponsePayload(payload));
...
function renderTokenChart(byDate = {}) {
  const chart = $('tokenChart');
  const dates = lastNDates(7);
  const rows = dates.map(date => ({ date, ...(byDate[date] || {}) }));

  if (!Object.keys(byDate || {}).length) {
    chart.innerHTML = '<div class="notice" style="width:100%">暂无按日期统计的 token 数据，发起请求后会显示每日趋势。</div>';
    return;
  }

  const width = 700, height = 190, padX = 38, top = 18, bottom = 34;
  const plotW = width - padX * 2;
  const plotH = height - top - bottom;
  const max = Math.max(1, ...rows.map(r => r.totalTokens || 0));

  const x = i => padX + (plotW / (rows.length - 1)) * i;
  const y = v => top + plotH - (v / max) * plotH;

  const points = rows.map((r, i) => [x(i), y(r.totalTokens || 0)]);
  const line = points.map(p => p.join(',')).join(' ');
  const area = padX + ',' + (top + plotH) + ' ' + line + ' ' + (padX + plotW) + ',' + (top + plotH);

  const grid = [0, 0.25, 0.5, 0.75, 1].map(t => {
    return '<line class="trend-grid" x1="' + padX +
      '" y1="' + (top + plotH * t) +
      '" x2="' + (padX + plotW) +
      '" y2="' + (top + plotH * t) + '"/>';
  }).join('');
  ...
  const dots = rows.map((r, i) => {
    ...
    return '<circle class="trend-dot" cx="' + cx +
      '" cy="' + cy +
      '" r="4"><title>' + r.date +
      ' · 总计 ' + fmt(total) +
      ' tokens · 输入 ' + fmt(r.inputTokens) +
      ' / 输出 ' + fmt(r.outputTokens) +
      '</title></circle>' +
      (total ? '<text class="trend-value" x="' + cx +
        '" y="' + Math.max(12, cy - 10) +
        '">' + fmt(total) + '</text>' : '') +
      '<text class="trend-label" x="' + cx +
      '" y="' + (height - 8) +
      '">' + r.date.slice(5) + '</text>';
  }).join('');
  ...
  chart.innerHTML =
    ...
    '" preserveAspectRatio="none"><defs><linearGradient id="tokenGradient" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#2563eb"/><stop offset="1" stop-color="#2563eb" stop-opacity="0"/>' +
...
function updateUsageStats(stats={}){ const requests=stats.totalRequests||0; const errors=stats.totalErrors||0; $('requestCount').textContent=fmt(requests); $('errorRate').textContent='错误率 '+(requests?((errors/requests)*100).toFixed(1):'0.0')+'%'; $('tokenTotal').textContent=fmt(stats.totalTokens); $('tokenSplit').textContent='输入 Token '+fmt(stats.totalInputTokens)+' / 输出 Token '+fmt(stats.totalOutputTokens)+(stats.estimatedRequests?' · 含估算 '+stats.estimatedRequests+' 次':''); const byModel=stats.byModel||{}; const activeModel=lastStatus?.providers?.find?.(p=>p.id===lastStatus.activeProvider)?.model || lastStatus?.activeProvider; const active=byModel[activeModel]||{}; $('activeModelTokens').textContent=fmt(active.totalTokens); $('activeModelName').textContent=activeModel||'-'; renderTokenChart(stats.byDate||{}); const rows=Object.entries(byModel).sort((a,b)=>(b[1].totalTokens||0)-(a[1].totalTokens||0)).slice(0,8); const max=Math.max(1,...rows.map(([,item])=>item.totalTokens||0)); $('modelUsage').innerHTML=rows.length?'':'<div class="notice">暂无模型用量数据，发起请求后会自动统计。</div>'; rows.forEach(([model,item])=>{ const el=document.createElement('div'); el.className='hbar-row'; el.innerHTML='<span class="code"></span><div class="hbar-track"><div class="hbar-fill"></div></div><strong></strong><div style="grid-column:1/-1;font-size:12px;color:#6b7280"></div>'; el.querySelector('.code').textContent=model; el.querySelector('.hbar-fill').style.width=Math.max(2,((item.totalTokens||0)/max)*100)+'%'; el.querySelector('strong').textContent=fmt(item.totalTokens||0); el.lastElementChild.textContent=(item.requests||0)+' 请求 · 输入 Token '+fmt(item.inputTokens||0)+' / 输出 Token '+fmt(item.outputTokens||0)+(item.estimatedRequests?' · 估算 '+item.estimatedRequests+' 次':''); $('modelUsage').append(el); }); }
...
const uiServer = http.createServer(async (req, res) => {
  try {
    const requestUrl = new URL(req.url || "/", `http://${API_HOST}:${UI_PORT}`);
